    ...
    <div class="p-12 lg:p-24" style="padding-bottom: 64px;">
        <div class="flex flex-col items-center  mb-6 h-full xl:flex-row">
            ...
            <div class="text-white flex flex-col items-start my-4">
                ...
                <div>
                    <el-button class="bg-white w-30 h-12 text-lg hover:text-red-800" type="defalult" round :icon="CaretRight" @click="addplaylist">播放</el-button>
                </div>
            </div>
        </div>
        <ul>
            <router-link :to="{path:'/playdetail',query:{id:item.id,pic:album.picurl,sn:item.name,sgn:album.singername,time:album.dt}}" v-for="(item,index) in album.songs" :key="item.id"  class="flex items-center justify-between w-full p-3 py-6 text-white border-b border-b-zinc-800 hover:text-red-800">
...
const addplaylist = () => {
    // setSongList.songList = album.songs 
